...
static void stampa_giocatori()
{
  ...
  {
    printf("\nGIOCATORE %d -> %s -> %c",w+1,giocatori[w].nome,giocatori[w].stato);
  }
}
...
static void chiamata_emergenza()
{
  ...
  for(int i=0;i<numeroGiocatori;i++)
  {
    ...
    if(giocatori[c].posizione==giocatori[i].posizione && giocatori[i].stato==0 && flag==false)
    {
      ...
      {
        giocatori[i].stato=3;
        printf("\nIl giocatore %c e' stato defenestrato ed era un ASTRONAUTA\n", giocatori[i].nome);
        numeroMorti++;
    ...
    if(giocatori[c].posizione==giocatori[i].posizione && giocatori[i].stato==1 && flag==false)
    {
      ...
      {
        giocatori[i].stato=3;
        printf("\nIl giocatore %c e' stato defenestrato ed era un IMPOSTORE\n", giocatori[i].nome);
        numeroDefenestrati++;
        flag=true; //FLAG IL QUALE PERMETTE CHE UN SOLO GIOCATORE SIA DEFENSTRATO
      }
    }
...
static void uccidi_astronauta()
{
  ...
  if(nAstronautiQui==1)
  {
    for(int i=0;i<numeroGiocatori && ucciso==0;i++)
    {
      if(giocatori[i].posizione==giocatori[c].posizione && giocatori[i].stato==0)
      {
        printf("Hai ucciso il giocatore %c ", giocatori[i].nome);
        giocatori[i].stato=2;
      }
    }
  }
          ...
          do
          {
            ...
            if(contantore==random && giocatori[c].posizione!=pScan)
            {
              ...
              else
              {
                ...
                {
                  if(giocatori[i].posizione==pScan && giocatori[i].nome!=giocatori[c].nome && giocatori[i].stato!=2 && giocatori[i].stato!=3)
                    printf("\t- %c (%c)\n", giocatori[i].nome,giocatori[i].stato);
                }
              }
              flag=1;
            }
...
bool impostagioco()
{
  ...
  do
  {
    printf("\nQUANTE QUEST DOVRANNO COMPIERE GLI ASTRONAUTI PER POTER VINCERE?\nil numero di quest deve essere almeno il doppio del numero dei giocatori (esempio: i giocatori sono 4, le quest dovranno essere minimo 8)\n");
    scanf("%hu",&quest_da_finire);
    if(quest_da_finire<numeroGiocatori*2)
      printf("\nATTENZIONE! NUMERO QUEST TROPPO BASSO! TI RICORDO CHE DEVE ESSERE ALMENO IL DOPPIO DEL NUMERO DI GIOCATORI!");
    system("clear");
  } while(quest_da_finire<numeroGiocatori*2);
  ...
  //PULIZIA DELLO SCHERMO E RITORNO DELLA VARIABILE
  system("clear");
  bool a=true;
  return a;
}
...
void gioca()
{
  ...
  do {
    ...
    switch (giocatori[c].stato)
    {
      ...
      case 1: //IMPOSTORI
        system("clear");
        printf("\nTOCCA AL GIOCATORE %c\n",giocatori[c].nome);
        printf("\nLA STANZA ALL'INTERNO DELLA QUALE TI TROVI E' DI TIPO %c\n",giocatori[c].posizione->tipo);
